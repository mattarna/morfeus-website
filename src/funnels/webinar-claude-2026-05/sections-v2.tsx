...
import type {
  SalesV2FinalCTAContent,
  SalesV2HeroContent,
  SalesV2PricingContent,
  SalesV2UrgencyContent,
  SalesV2Variant,
      ...
      <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 16, maxWidth: 820 }}>
        {points.map((p, i) => (
          ...
          >
            ...
            <span style={{ fontFamily: "var(--font-body)", fontSize: 19, lineHeight: 1.5, color: "var(--ghost)", opacity: 0.92 }}>
              {p}
            </span>
          </li>
        ))}
    ...
    >
      ...
      <div className={styles.hostGrid}>
        ...
        <div>
          ...
          <p style={{ fontFamily: "var(--font-body)", fontSize: 18, fontWeight: 500, color: "var(--ghost)", opacity: 0.7, margin: "0 0 32px 0" }}>
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
        ...
        <h3 style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 19, lineHeight: 1.2, letterSpacing: "-0.015em", color: "#fff", margin: 0 }}>
          Condividi con chi pensi possa servire
        </h3>
      </div>
      <p style={{ fontFamily: "var(--font-body)", fontSize: 17, lineHeight: 1.55, color: "var(--ghost)", opacity: 0.80, margin: "0 0 18px 0" }}>
 ...
 * (sezione webinar). PrimaryButton webinar è separato dal SalesV2PrimaryButton
 * — quest'ultimo aggiunge supporto href anchor + size "xl".
 * ═══════════════════════════════════════════════════════════════════════════════ */

// ─── Variant hook ─────────────────────────────────────────────────────────────

function isVariant(v: string | null): v is SalesV2Variant {
  return v === "live" || v === "replay" || v === "email";
}

function useSalesV2Variant(): SalesV2Variant {
  const [variant, setVariant] = useState<SalesV2Variant>("replay");
  useEffect(() => {
    if (typeof window === "undefined") return;
    const sp = new URLSearchParams(window.location.search);
...
function useCurrentPricing(pricing: SalesV2PricingContent): CurrentPricing {
...
function trackCheckoutClick(block: string, variant: SalesV2Variant, stage: PricingStage, price: number) {
...
function SalesV2PageVariantTracker() {
...
export function SalesV2HeaderSection() {
  const [isMobile, setIsMobile] = useState(false);
...
export function SalesV2HeroSection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const content = step.content.SalesV2Hero as SalesV2HeroContent;
...
export function SalesV2ThreeLevelsSection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const pricing = step.content.SalesV2Pricing as SalesV2PricingContent;
  const current = useCurrentPricing(pricing);
  const onCheckout = () => {
...
export function SalesV2PromiseLetterSection() {
    ...
    >
      <div style={{ maxWidth: 760, margin: "0 auto", position: "relative" }}>
        ...
        >
          ...
          >
            <strong style={{ fontStyle: "normal", fontWeight: 600, fontFamily: "var(--font-body)", fontSize: 17, letterSpacing: "0.08em", textTransform: "uppercase", color: "#6B5E54", marginRight: 10 }}>
              P.S.
            </strong>
...
export function SalesV2ModulesSection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const pricing = step.content.SalesV2Pricing as SalesV2PricingContent;
  const current = useCurrentPricing(pricing);
  const onCheckout = () => {
...
export function SalesV2ProofSection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const pricing = step.content.SalesV2Pricing as SalesV2PricingContent;
  const current = useCurrentPricing(pricing);
  const onCheckout = () => {
 ...
 */
export function SalesV2ReviewsSection() {
...
export function SalesV2AudienceSection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const pricing = step.content.SalesV2Pricing as SalesV2PricingContent;
  const current = useCurrentPricing(pricing);
  const onCheckout = () => {
      ...
      <div className={styles.audienceGrid}>
        ...
        >
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 16 }}>
            {yes.map((item, i) => (
              <li key={i} style={{ display: "flex", gap: 14, fontFamily: "var(--font-body)", fontSize: 18, lineHeight: 1.55, color: "var(--ghost)" }}>
                <span
                  style={{
                    flexShrink: 0,
                    width: 22,
        ...
        >
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 16 }}>
            {no.map((item, i) => (
              <li key={i} style={{ display: "flex", gap: 14, fontFamily: "var(--font-body)", fontSize: 18, lineHeight: 1.55, color: "var(--ghost)", opacity: 0.85 }}>
                <span
                  style={{
                    flexShrink: 0,
                    width: 22,
...
export function SalesV2ComparisonSection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const pricing = step.content.SalesV2Pricing as SalesV2PricingContent;
  const current = useCurrentPricing(pricing);
  const onCheckout = () => {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {rows.map((r) => (
          <div key={r.k}>
            ...
            <div style={{ fontFamily: "var(--font-body)", fontSize: 18, lineHeight: 1.55, color: "var(--ghost)" }}>{r.v}</div>
          </div>
        ))}
      </div>
    </div>
...
export function SalesV2BonusSection() {
  const bonuses = [
    {
      tag: "Bonus #1",
...
export function SalesV2OfferSection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const pricing = step.content.SalesV2Pricing as SalesV2PricingContent;
  const current = useCurrentPricing(pricing);
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
          <div style={{ fontFamily: "var(--font-body)", fontSize: 18, color: "var(--ghost)", lineHeight: 1.6 }}>
      ...
      >
        ...
        <div style={{ fontFamily: "var(--font-body)", fontSize: 18, lineHeight: 1.55, color: "var(--ghost)" }}>
  ...
  return (
    ...
    >
      ...
      <div className={styles.offerStackBody}>
        <div style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 19, color: "#fff", marginBottom: 4, lineHeight: 1.25 }}>
          {title}
        </div>
        <div style={{ fontFamily: "var(--font-body)", fontSize: 17, lineHeight: 1.5, color: "var(--ghost)", opacity: 0.8, whiteSpace: "pre-line" }}>
          {body}
        </div>
      </div>
      <div
...
export function SalesV2UpsellBootcampSection() {
  const variant = useSalesV2Variant();
...
export function SalesV2GuaranteeSection() {
  return (
    <section
      className={styles.salesSectionPadSm}
      style={{
...
export function SalesV2FAQSection() {
  const variant = useSalesV2Variant();
...
export function SalesV2BootcampBridgeSection() {
  const variant = useSalesV2Variant();
...
export function SalesV2UrgencySection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const content = step.content.SalesV2Urgency as SalesV2UrgencyContent;
  const pricing = step.content.SalesV2Pricing as SalesV2PricingContent;
  const current = useCurrentPricing(pricing);
  const copy = content[variant];
...
export function SalesV2FinalCTASection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const content = step.content.SalesV2FinalCTA as SalesV2FinalCTAContent;
  const pricing = step.content.SalesV2Pricing as SalesV2PricingContent;
  const current = useCurrentPricing(pricing);
  const copy = content[variant];
...
export function SalesV2B2BSection() {
  const variant = useSalesV2Variant();
...
export function SalesV2FooterSection() {
  const muted: React.CSSProperties = {
    fontFamily: "var(--font-body)",
    fontSize: 11,
    lineHeight: 1.6,
...
export function SalesV2StickyBarSection({ step }: SectionProps) {
  const variant = useSalesV2Variant();
  const pricing = step?.content?.SalesV2Pricing as SalesV2PricingContent | undefined;
