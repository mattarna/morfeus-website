# Modulo 05 — Da Agenzia a Incubatore: La Formula Partner per Lanciare Business da Zero
...
| Durata | 43 min |
| Tags | modello partner, incubatore, bootstrapping, caso studio fisioterapia, contenuti educativi, hiring venditori, Agora, Golden Hippo |

[← Modulo precedente](./04-federico-creativita-advertising.md) | [Torna all'indice](../index.md) | [Modulo successivo →](./06-simone-vendita-etica.md)
...
[+] **L'approccio di Soluzione ai contenuti (dal principio di Marco):**
Il loro "algoritmo" non è quello di YouTube o Instagram. Il loro algoritmo sono le domande e le obiezioni che arrivano dalla rete vendita. Processo:
1. **Il venditore segnala un'obiezione ricorrente** (es. "i miei pazienti non pagano abbastanza")
2. **Si produce un video/episodio del podcast** che affronta quel tema in profondità
3. **Il video serve a due scopi:** (a) educa il potenziale cliente prima della call, accorciando il ciclo di vendita, (b) il venditore può inviarlo durante la trattativa come "prova" che il metodo è solido
4. **I contenuti NON puntano alla viralità:** Marco lo dice esplicitamente: "sono dei video che all'apparenza non hanno un potenziale di viralità o espansione. Non ci interessa fare quel tipo di contenuti. Ci interessa fare contenuti che aiutano il lead a valere di più."
...
[+] **Perché il verticale è un "salto con l'imbracatura" (metafora di Marco):**
Nel mercato orizzontale, ogni progetto è un salto nel vuoto. Hai bisogno di "campioni del mondo" perché ogni cliente richiede competenze diverse. Nel mercato verticale:
...
1. **Annuncio mirato:** non generico "cerchiamo venditori". Specifico per il verticale (es. venditori per il settore sanitario).
2. **Volume di candidature:** hanno raccolto 150 curriculum in pochi mesi. Il volume alto è intenzionale: più candidati = più possibilità di trovare i giusti.
3. **Panel di reclutamento a più livelli:** non un singolo colloquio ma una serie di step progressivi. Ogni livello filtra: competenze, attitudine, allineamento con i valori, capacità di vendita consultiva.
4. **Selezione finale:** dai 150 CV, selezionati 2 venditori. Tasso di selezione: 1,3%. Questo è intenzionale: meglio 2 venditori eccellenti che 10 mediocri.
5. **Ispirazione:** hanno seguito il consiglio del panel di reclutamento strutturato (citazione nell'intervento di un metodo specifico).
...
[+] **Cosa costituisce una "cucina" forte (dal modello Soluzione):**
La "cucina" di Soluzione non è un singolo skill ma un SISTEMA integrato:
...
**I primi 90 giorni:**
1. **Mese 1:** costruisci il team, lancia il funnel di generation, ascolta il mercato
2. **Mese 2:** adatta l'offerta in base al feedback, avvia la produzione contenuti, configura la rete vendita (direttore vendita full-time!)
3. **Mese 3:** scala la generation (più ads, più angoli), inizia a monetizzare, misura i KPI
...
**KPI della macchina contenuti:**
Non misuri views o follower. Misuri: i lead che hanno consumato i contenuti convertono meglio? Il ciclo di vendita si accorcia? Il ticket medio aumenta? Se sì a tutti e tre, la macchina funziona.
...
[+] **Perché il timing della transizione è critico (dall'esperienza di Marco):**
Marco lo dice esplicitamente: "prima che il personal brand sia così forte da trasmettere la propria autorevolezza per interposta persona... ci vuole tempo. Le persone vogliono avere a che fare con quella persona. Con i soliti è 1-to-1, e lo step successivo di scalata è quasi 1-to-1 ma con quella persona. Se la togli troppo presto e troppo spesso dall'equazione, poi non funziona."

**La sequenza di transizione:**
1. **Fase 1 (mesi 1-6):** tutto 1-to-1. Il partner eroga, il brand cresce, i risultati si accumulano
2. **Fase 2 (mesi 6-12):** 1-to-1 con supporto tutor. Il partner resta il punto di contatto principale ma inizia a delegare parti dell'erogazione
3. **Fase 3 (mesi 12+):** mini-classi. Il brand è abbastanza forte che i clienti accettano un formato di gruppo. Il partner resta il "volto" ma non eroga ogni singola sessione
4. **Fase 4 (mesi 18+):** corsi e prodotti scalabili. Solo quando il brand è consolidato e i risultati dei clienti sono documentati
...
**Come è partito:**
1. Costruzione team + accesso al network di Soluzione
2. Lancio campagna di generation (guida gratuita + VSL)
3. Pivot in 24 ore (da consulenze a campagne di acquisizione pazienti)
4. Avvio YouTube: prima live (non funzionano), poi podcast ("Il Podcast Sanitario")
5. Hiring venditori: 150 CV raccolti, 2 selezionati (tasso 1,3%)
...
|--------|----------------|
| [01 - Marco](./01-marco-regole-infobusiness.md) | Il modello partner è l'implementazione operativa dei 4 pilastri. La generation è il pilastro 1, i contenuti sono il pilastro 3, il cash flow critico è il contesto |
| [02 - Michele](./02-michele-offerte-conversione.md) | L'hiring dei venditori richiede la pre-qualifica di Michele. Il panel di reclutamento segue il modello Kennedy. Il feedback del mercato (pivot in 24h) è ascolto attivo della domanda |
| [03 - Roberto](./03-roberto-war-machine.md) | Il funnel di generation del caso Tosini (guida + VSL) è il Layer 2 di Roberto. La progressione generation → harvest segue la logica Layer 2 → Layer 1 → Layer 3 |
| [04 - Federico](./04-federico-creativita-advertising.md) | I contenuti per vendita (Marco) sono complementari ai contenuti per algoritmo (Federico). L'approccio di Marco è ideale per chi parte da zero, quello di Federico per chi ha già un brand e vuole ottimizzare |
| [06 - Simone](./06-simone-vendita-etica.md) | La rete vendita e il direttore vendita full-time trovano nel modello di Simone il framework etico per le conversazioni di chiusura |
| [07 - Michelangelo](./07-michelangelo-meta-ads.md) | Le 3 ads attive del caso Tosini per la generation sono gestite con il framework Meta di Michelangelo |
