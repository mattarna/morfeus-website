...
/* Non tutto quello che ha un href e' una pagina del sito.
   - sotto /public vivono i pacchetti statici (corsi, lezioni, playbook):
     sono documenti a se', il router di Next non li conosce e una
     router.push() li' finirebbe su un 404 lato client;
   - /playground e' il sito gemello sul sottodominio, con un suo guscio;
   - i funnel stanno su /<slug> ma internamente sono /funnel-internal.
   I prefissi delle cartelle versionate sono troncati apposta, cosi'
   coprono anche /corso-claude-unlocked-a6c95d9c6f e simili. */
const FUORI_DAL_GUSCIO = [
  "/api",
  "/funnel-internal",
  "/__funnels",
  "/playground",
  ...
  "/fonts",
  "/logo",
  "/loghi_bianchi_clienti",
];

function passaDalRouter(pathname: string): boolean {
  if (FUORI_DAL_GUSCIO.some((prefisso) => pathname.startsWith(prefisso))) return false;
