...
export function PreCallSectionShell({ 
  ...
  return (
    <section
      id={id}
      className={`relative z-10 border-y border-white/5 ${bgClass} px-6 py-20 shadow-[inset_0_0_150px_rgba(0,0,0,0.6)] md:px-10 md:py-24 xl:px-40 ${className}`}
    >
      {showGrid && <GridPattern />}
      
      {number && (
        <div className="absolute left-1/2 -translate-x-1/2 top-0 pointer-events-none select-none">
          <span className="text-[12rem] md:text-[18rem] font-black text-white/2 font-mono tracking-tighter">
            {number}
          </span>
        </div>
      )}
