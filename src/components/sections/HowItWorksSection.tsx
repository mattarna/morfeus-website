...
export function HowItWorksSection({
  ...
  return (
    <section id="how-it-works" className="relative z-60 bg-night border-t border-white/5 overflow-visible">
      <GridPattern />
      ...
      <div className="relative z-10 py-24 md:py-40 px-6 xl:px-20 max-w-[1400px] mx-auto">
        ...
        <div className="relative">
          {/* Main Schematic Line */}
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-px bg-slate-800 md:-translate-x-1/2 hidden md:block" />
            ...
            {stepIds.map((id, idx) => (
              ...
              >
                ...
                <div className={`absolute left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 ${(idx + 1) % 2 === 0 ? 'md:right-1/2 md:mr-24' : 'md:left-1/2 md:ml-24'} top-[-30px] md:top-[-60px]`}>
                  <span className="text-8xl md:text-[12rem] font-black text-white/4 font-mono tracking-tighter">
                    0{id}
                  </span>
                </div>
                ...
                <div className={`w-full md:w-[calc(50%-3rem)] text-center ${(idx + 1) % 2 === 0 ? 'md:text-right' : 'md:text-left'}`}>
                  <div className="relative">
                    {/* Technical ID Tag */}
                    <div className={`inline-flex items-center gap-3 mb-6 px-3 py-1 rounded-sm border border-indigo-500/30 bg-indigo-500/5 ${(idx + 1) % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                    ...
                    <ul className={`flex flex-col gap-6 mb-12 items-center ${(idx + 1) % 2 === 0 ? 'md:items-end' : 'md:items-start'}`}>
                      {(Object.values(t.raw(`steps.${id}.bullets`)) as string[]).map((bullet, i) => (
                        <li key={i} className={`flex items-start gap-4 text-slate-200 font-light group text-left ${(idx + 1) % 2 === 0 ? 'md:flex-row-reverse md:text-right' : ''}`}>
                          <span className="mt-2.5 w-2 h-2 rounded-xs rotate-45 border border-indigo-500/50 group-hover:bg-indigo-500 transition-all duration-300 shrink-0" />
        ...
        {/* MARF Box */}
        {showMarfBox && (
          <div className="mt-40 p-8 md:p-12 lg:p-16 rounded-[2.5rem] bg-linear-to-br from-[#0d1117] to-[#0a0c10] border border-indigo-500/30 shadow-[0_20px_50px_-20px_rgba(83,61,252,0.3)] relative overflow-hidden group transition-all duration-500 hover:border-indigo-500/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
              {(["1", "2", "3", "4"] as const).map((key) => {
                ...
                return (
                  <div key={key} className="flex gap-4 p-5 rounded-2xl bg-white/2 border border-white/5 hover:border-indigo-500/20 transition-all duration-300 relative group/bullet hover:bg-white/4">
                    <div className="shrink-0 w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center relative transition-colors group-hover/bullet:border-forge/40">
        ...
        <div className="mt-32 md:mt-48 flex flex-col items-center gap-12">
          <div className="h-px w-24 bg-linear-to-r from-transparent via-indigo-500 to-transparent mb-4" />
          <button 
            onClick={scrollToContact}
