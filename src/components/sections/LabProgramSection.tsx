...
export function LabProgramSection({ namespace = "Lab.program" }: LabProgramSectionProps) {
  ...
  return (
    <section id="ai-champ" className="relative z-100 py-24 md:py-40 px-6 xl:px-40 bg-[#0a111a] border-t border-white/5 shadow-[inset_0_0_150px_rgba(0,0,0,0.6)] overflow-visible">
      <div className="relative z-10 max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-16 md:mb-24">
          {(["1", "2"] as const).map((key) => (
            <div key={key} className="text-center md:text-left p-8 md:p-12 rounded-[2.5rem] bg-white/2 border border-white/5">
              <div className="text-6xl md:text-8xl font-black tracking-[-0.04em] leading-none bg-clip-text text-transparent bg-linear-to-b from-white via-slate-200 to-slate-400 mb-4">
                {t(`stats.${key}.number`)}
              </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-16 md:mb-24">
          {tags.map((tag) => (
            <span key={tag} className="text-[10px] font-bold tracking-[0.08em] uppercase text-vista bg-indigo-500/8 px-4 py-2 rounded-lg border border-indigo-500/10">
              {tag}
            </span>
          ))}
        </div>
