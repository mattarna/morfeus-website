/* ============================================================
   FORGE · la copy, spostata qui dalla pagina.
   ------------------------------------------------------------
   Testo INVARIATO rispetto alla versione precedente: questo giro
...
/* L'unione delle due lingue, non il solo italiano: con `as const` i tipi
   sono LETTERALI, quindi COPY.en non e' assegnabile a typeof COPY["it"]
   (stessa forma, stringhe diverse). L'unione le accetta entrambe e in
   lettura si comporta come ci si aspetta. */
export type ForgeCopy = (typeof COPY)[keyof typeof COPY];
export type ForgeLocale = keyof typeof COPY;
