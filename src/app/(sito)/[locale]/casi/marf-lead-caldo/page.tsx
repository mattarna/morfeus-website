...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: t.metaDesc,
      type: "article",
      url: `${SITE_URL}${localePrefix(safeLocale)}/casi/marf-lead-caldo`,
      siteName: "Morfeus",
...
export default async function CasoMarfLeadCaldoPage({ params }: Props) {
  const { locale } = await params;
  const isIt = locale === "it";
  const t = isIt ? COPY.it : COPY.en;
  const safeLocale: "it" | "en" = isIt ? "it" : "en";
  const base = `/${safeLocale}`;
  const url = `${SITE_URL}${localePrefix(safeLocale)}/casi/marf-lead-caldo`;
